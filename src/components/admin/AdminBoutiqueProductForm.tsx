...
export default function AdminBoutiqueProductForm({ product, initialCategories = [] }: Props) {
  ...
  const [modelRotation, setModelRotation] = useState<{ x: number; y: number; z: number }>(
    product?.model_rotation ?? { x: 0, y: 0, z: 0 }
  )
          ...
          {stlUrl && (
            <div>
              ...
              <div className="rounded-2xl overflow-hidden border border-[var(--line)] bg-bg-1 mb-4" style={{ height: 240 }}>
                <STLViewerWrapper url={stlUrl} height={240} rotation={modelRotation} />
              </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                {[0, 90, 180, 270].map((deg) => (
                  <button
                    key={deg}
                    type="button"
                    onClick={() => setModelRotation((r) => ({ ...r, y: deg }))}
                    className={[
                ...
                {(['x', 'y', 'z'] as const).map((axis) => (
                  <div key={axis} className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) => setModelRotation((r) => ({ ...r, [axis]: parseInt(e.target.value, 10) }))}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!isNaN(v)) setModelRotation((r) => ({ ...r, [axis]: Math.max(-180, Math.min(180, v)) }))
                      }}
