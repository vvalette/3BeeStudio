...
- **Fiche admin, deux cartes qui ne se marchent plus dessus** : « Devis » porte le document et ses montants (total, acompte, objet, numéro) avec deux gestes, **Enregistrer sans envoyer** (pose total et acompte sur la demande, rien ne part au client) ou **Envoyer au client** ; « Paiements » porte l'argent, réclamation du solde comprise. Les deux gestes existent dans les deux onglets, composeur comme import : sans « Enregistrer », les montants tapés dans le composeur restaient en mémoire du navigateur et la carte « Paiements » affichait « aucun acompte défini »
- **Carte « Paiements », lecture de haut en bas** : récapitulatif d'abord (total, encaissé, reste dû), puis les échéances. Le solde n'est plus verrouillé derrière l'encaissement de l'acompte, sinon un premier versement reçu hors de l'app fermait la carte sur un cul-de-sac. Acompte égal au total = « Réglé en une fois », pas de solde affiché. Chaque formulaire de déclaration propose les montants déjà connus de la fiche en un clic (acompte prévu, reste dû, total)
- **Fiche corrigeable en place** (`AdminCustomFields`) : « Modifier » dans l'en-tête des cartes « Projet », « Client » et « Expédition ». Un bloc à la fois. Une adresse absente s'ajoute depuis la même carte (« Ajouter »), l'étiquette Boxtal n'apparaissant qu'une fois l'adresse complète. Colonnes NOT NULL (`name`, `email`, `project_type`, `description`) refusées vides, colonnes nullables vidées à `null` et non à `''`
...
- **Paiement hors Stripe** : à l'envoi du devis comme de la demande de solde, choix « Lien de paiement » ou « Virement ». En virement, aucune session Stripe n'est créée (un lien ouvert finirait cliqué, double encaissement) et l'email annonce le montant sans bouton. L'admin déclare l'encaissement depuis la carte « Paiements » : montant, **date du relevé** (c'est elle qui compte pour la déclaration) et moyen (`deposit_method` / `balance_method` : `stripe`, `transfer`, `cash`, `check`), repris en colonne « Moyen d'encaissement » du CSV. Déclaration annulable en un clic tant qu'elle vient de l'admin. Une demande négociée ailleurs, dont l'acompte est **déjà encaissé**, se saisit directement là (rien n'est envoyé au client) : le formulaire réclame alors le **total du projet**, sans lequel le solde et la facture n'auraient pas de référence
...
- **Export CSV = CA réellement encaissé** : chaque ligne sur-mesure porte la date de SON règlement (`deposit_paid_at`, `balance_paid_at`), et une demande sans encaissement n'y figure pas
...
- **Coloris** (`shop_products.colors`, jsonb `[{key,label,label_en,hex}]`) : palette éditable par fiche dans l'admin, pastilles sur la fiche produit, premier coloris présélectionné. Le choix suit la ligne jusqu'à la commande (`shop_orders.items[].color`), les emails, la fiche admin et la facture. Une ligne = un produit **et** un coloris (`cartLineKey`) ; le stock reste commun au produit. Pas encore de photo par coloris
