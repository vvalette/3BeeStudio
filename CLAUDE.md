...
- **Paiement hors Stripe** : à l'envoi du devis comme de la demande de solde, choix « Lien de paiement » ou « Virement ». En virement, aucune session Stripe n'est créée (un lien ouvert finirait cliqué, double encaissement) et l'email annonce le montant sans bouton. L'admin déclare ensuite l'encaissement depuis la carte « Paiements » : montant, **date du relevé** (c'est elle qui compte pour la déclaration) et moyen (`deposit_method` / `balance_method` : `stripe`, `transfer`, `cash`, `check`), repris en colonne « Moyen d'encaissement » du CSV. Déclaration annulable en un clic tant qu'elle vient de l'admin
